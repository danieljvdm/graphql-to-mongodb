...
export interface FieldFilter {
  (
    name: string,
    field: { resolve?: Function; dependencies?: string[] }
  ): Boolean;
}
...
export function getUnresolvedFieldsTypes<T extends GraphQLType>(
  graphQLType: GraphQLObjectType,
  typeResolver: TypeResolver<T> = null,
  ...excludedFields: string[]
): () => FieldMap<T> {
  return () => {
    ...
    Object.keys(fields).forEach(
      key => (fieldsTypes[key] = { type: fields[key].type })
    );
    return fieldsTypes;
  };
}
...
export function clear(obj: object, ...excludedKeys: string[]): object {
  return Object.keys(obj).reduce((cleared, key) => {
    ...
    if (value !== undefined && value !== null && !excludedKeys.includes(key)) {
      if (typeof value != "object" || value instanceof Date || isType(value)) {
