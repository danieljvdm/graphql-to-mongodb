...
export function getGraphQLFilterType(
...
function getOrAndFields(
  type: GraphQLObjectType,
  ...excludedFields: string[]
): () => FieldMap<GraphQLInputType> {
  return () => {
    const generatedFields = getUnresolvedFieldsTypes(
      type,
    ...
    // @ts-ignore
    generatedFields["OR"] = {
      type: new GraphQLList(getGraphQLFilterType(type, ...excludedFields))
    };
    // @ts-ignore
    generatedFields["AND"] = {
      type: new GraphQLList(getGraphQLFilterType(type, ...excludedFields))
...
function getInputObjectTypeFields(
  type: GraphQLObjectType,
  ...excludedFields: string[]
): () => FieldMap<GraphQLInputType> {
  return () => {
...
function getGraphQLScalarFilterType(
  scalarType: GraphQLScalarType | GraphQLEnumType
): GraphQLInputObjectType {
  const typeName = scalarType.toString() + "Filter";

  return cache(
    scalarFilterTypesCache,
    typeName,
    () =>
      new GraphQLInputObjectType({
        name: typeName,
...
function getGraphQLScalarFilterTypeFields(
  scalarType: GraphQLScalarType | GraphQLEnumType
) {
  return {
    opr: {
...
function warnIndependentResolveFields(type: GraphQLObjectType): void {
  cache(warnedIndependentResolvers, type.toString(), () => {
    const fields = getTypeFields(
      type,
      (key, field) => field.resolve && !Array.isArray(field.dependencies)
    )();

    Object.keys(fields).forEach(key =>
      warn(
        `Field ${key} of type ${type} has a resolve function and no dependencies`
      )
    );
