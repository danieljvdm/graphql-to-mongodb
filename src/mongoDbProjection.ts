...
function getMongoDbProjection(
  ...
  return logOnError(() => {
    ...
    const resolveFieldsDependencies = [].concat(
      ...Object.keys(resolveFields).map(_ => resolveFields[_])
    );
    return mergeProjectionAndResolveDependencies(
      projection,
      resolveFieldsDependencies
    );
  });
}

function mergeAndSimplifyNodes(nodes: FieldNode[]): object {
  const getFieldNodeFields = (fieldNode: FieldNode): FieldNode[] =>
    fieldNode.selectionSet.selections
      .filter(_ => _.kind == "Field")
      .map(_ => _ as FieldNode);
  const getFieldName = (fieldNode: FieldNode): string => fieldNode.name.value;
  const isFieldNodeScalar = (fieldNode: FieldNode): boolean =>
    !fieldNode.selectionSet;

  const nodesFields: { [argName: string]: FieldNode }[] = nodes
    .map(getFieldNodeFields)
    .map(_ =>
      _.reduce((seed, curr) => ({ ...seed, [getFieldName(curr)]: curr }), {})
    );
    ...
    ...Object.keys(fieldNode)
      ...
      .map(key => {
        ...
        return getProjection(
          field,
          getInnerType(typeFields[key].type) as GraphQLObjectType,
          newPath
        );
      })
  );
  ...
  return Object.assign(
    ...
    ...Object.keys(fieldNode)
      ...
      .map(key => {
        ...
        return getResolveFields(
          field,
          getInnerType(typeField.type) as GraphQLObjectType,
          newPath
        );
      })
  );
...
function mergeProjectionAndResolveDependencies(
  projection: object,
